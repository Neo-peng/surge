...
(() => {
  ...
  if (magicJS.isResponse) {
    switch (true) {
      // 回答内容优化
      case /^https?:\/\/www\.zhihu\.com\/appview\/v2\/answer\/.*(entry=(?!(preload-topstory|preload-search|preload-subscription)))?/.test(
        magicJS.request.url
      ):
        try {
      ...
      case /^https:\/\/api\.zhihu\.com\/people\/self/.test(magicJS.request.url):
        try {
          ...
          magicJS.logDebug(
            `用户登录用户信息，接口响应：${magicJS.response.body}`
          );
          if (
            obj &&
            obj['id'] &&
        ...
        break;
      // 去除MCN信息
      case /^https?:\/\/api\.zhihu\.com\/people\/((?!self).)*$/.test(
        magicJS.request.url
      ):
        try {
          let obj = JSON.parse(magicJS.response.body);
      ...
      case /^https:\/\/api\.zhihu\.com\/topstory\/recommend\?/.test(
        ...
        try {
          ...
          function IsYanXuan(element) {
            ...
            if (element['common_card']['footline'].hasOwnProperty('elements')) {
              for (
                let i = 0;
                i < element['common_card']['footline']['elements'].length;
                i++
              ) {
                ...
                if (
                  ...
                  item['icon']['image_url'] ==
                    'https://pic2.zhimg.com/80/v2-c46fc8ec4c4e9ffc8f846ae0d8158a80_1440w.png'
                ) {
                  flag = true;
                  break;
                }
          ...
          let data = obj['data'].filter((element) => {
            ...
            try {
              if (
                element.hasOwnProperty('extra') &&
                element['extra'].hasOwnProperty('type') &&
                element['extra']['type'] === 'zvideo'
              ) {
                let video_id = element['common_card']['feed_content']['video'][
                  'customized_page_url'
                ].match(
                  /https?:\/\/www\.zhihu\.com\/[^?]*\?(videoID|content_id)=(\d*)/
                )[1];
                element['common_card']['feed_content']['video']['id'] =
                  video_id;
              }
            } catch (err) {
              magicJS.logError(
            ...
            try {
              if (
                flag === true &&
                customBlockedUsers &&
                element['common_card']['feed_content'].hasOwnProperty(
                  'source_line'
                ) &&
                element['common_card']['feed_content'][
                  'source_line'
                ].hasOwnProperty('elements') &&
                customBlockedUsers[
                  element['common_card']['feed_content']['source_line'][
                    'elements'
                  ][1]['text']['panel_text']
                ]
              ) {
        ...
        break;
      // 关注列表去广告
      case /^https?:\/\/api\.zhihu\.com\/moments(\/|\?)?(recommend|action=|feed_type=)(?!\/people)/.test(
        magicJS.request.url
      ):
        try {
          let obj = JSON.parse(magicJS.response.body);
          ...
          const targetIdFix = (element) => {
            if (element['target_type'] == 'pin') {
              target_id = element['target']['url'].match(
              ...
              if (
                ...
                element['target']['origin_pin'].hasOwnProperty('url')
              ) {
                origin_target_id = element['target']['origin_pin']['url'].match(
                  /https?:\/\/www\.zhihu\.com\/pin\/(\d*)/
                )[1];
        ...
        break;
      // 回答列表去广告与黑名单增强
      case /^https?:\/\/api\.zhihu\.com\/v4\/questions/.test(
        magicJS.request.url
      ):
        try {
          let userInfo = GetUserInfo();
          ...
          let obj = JSON.parse(magicJS.response.body);
          magicJS.logDebug(
            `当前黑名单列表: ${JSON.stringify(customBlockedUsers)}`
          );
          delete obj['ad_info'];
        ...
        break;
      // 拦截官方账号推广消息
      case /^https?:\/\/api\.zhihu\.com\/notifications\/v3\/timeline\/entry\/system_message/.test(
        magicJS.request.url
      ):
        try {
        ...
        break;
      // 屏蔽官方营销消息
      case /^https?:\/\/api\.zhihu\.com\/notifications\/v3\/message/.test(
        magicJS.request.url
      ):
        try {
          let obj = JSON.parse(magicJS.response.body);
        ...
        break;
      // 黑名单管理
      case /^https?:\/\/api\.zhihu\.com\/settings\/blocked_users/.test(
        magicJS.request.url
      ):
        let userInfo = GetUserInfo();
        ...
        if (magicJS.request.method == 'GET') {
          try {
            ...
            if (!!obj['data']) {
              ...
              magicJS.logWarning(
                `获取黑名单失败，接口响应不合法：${magicJS.response.body}`
              );
            }
          } catch (err) {
        ...
        // 写入黑名单
          ...
          try {
            ...
            if (obj.hasOwnProperty('name') && obj.hasOwnProperty('id')) {
              ...
              magicJS.logWarning(
                `写入黑名单失败，接口响应不合法：${magicJS.response.body}`
              );
        ...
        // 移出黑名单
          ...
          try {
            let obj = JSON.parse(magicJS.response.body);
            if (obj.success) {
              let user_id = magicJS.request.url.match(
              ...
              magicJS.logWarning(
                `移出黑名单失败，接口响应不合法：${magicJS.response.body}`
              );
        ...
        break;
      // 去除预置关键字广告
      case /^https?:\/\/api\.zhihu\.com\/search\/preset_words\?/.test(
        magicJS.request.url
      ):
        try {
          if (!!magicJS.response.body) {
            ...
            let obj = JSON.parse(magicJS.response.body);
            if (
              obj.hasOwnProperty('preset_words') &&
        ...
        break;
      // 优化知乎软件配置
      case /^https?:\/\/appcloud2\.zhihu\.com\/v\d+\/config/.test(
        magicJS.request.url
      ):
        try {
          if (!!magicJS.response.body) {
            let obj = JSON.parse(magicJS.response.body);
            let tab_infos = obj['config']['homepage_feed_tab'][
              'tab_infos'
            ].filter((e) => {
        ...
        break;
      // 知乎热搜去广告
      case /^https?:\/\/api\.zhihu\.com\/search\/top_search\/tabs\/hot\/items/.test(
        magicJS.request.url
      ):
        try {
          if (!!magicJS.response.body) {
        ...
        break;
      // 知乎热榜去广告
      case /^https?:\/\/api\.zhihu\.com\/topstory\/hot-lists?(\?|\/)/.test(
        magicJS.request.url
      ):
        try {
          if (!!magicJS.response.body) {
            ...
            let data = obj['data'].filter((e) => {
              return (
                e['type'] === 'hot_list_feed' ||
                e['type'] === 'hot_list_feed_video'
              );
            });
            obj['data'] = data;
            response = { body: JSON.stringify(obj) };
      ...
      case /^https?:\/\/api\.zhihu\.com\/comment_v5\/(answers|pins|comments?|articles)\/\d+\/(root|child)_comment/.test(
        ...
        try {
          if (!!magicJS.response.body) {
            ...
            // 屏蔽黑名单用户
            let user_info = GetUserInfo();
            let customBlockedUsers = magicJS.read(
              blockedUsersKey,
              user_info.id
            );
            customBlockedUsers = !!customBlockedUsers ? customBlockedUsers : {};
            ...
            obj.data.forEach((comment) => {
              ...
              if (
                ...
                if (
                  customBlockedUsers[commentUserName] &&
                  !replyUserName &&
                  magicJS.request.url.indexOf('root_comment') > 0
                ) {
                  magicJS.notifyDebug(
                  ...
                  customBlockedUsers[commentUserName] &&
                  !replyUserName &&
                  magicJS.request.url.indexOf('child_comment') > 0
                ) {
                  magicJS.notifyDebug(
                  ...
                  comment.child_comments.forEach((childComment) => {
                    ...
                    if (
                      ...
                      if (customBlockedUsers[childCommentUserName]) {
                        ...
                        blockCommentIdObj[childComment.id] =
        ...
        break;
      // 知乎旧版回答中的评论黑名单增强
      case /^https?:\/\/api\.zhihu\.com\/(answers|pins|comments?|articles)\/\d+\/(root|child)_comments/.test(
        magicJS.request.url
      ):
        try {
          if (!!magicJS.response.body) {
                ...
                if (comment.child_comments) {
                  ...
                  comment.child_comments.forEach((childComment) => {
                    if (
                      ...
                      customBlockedUsers[
                        childComment.reply_to_author.member.name
                      ]
                    ) {
      ...
      try {
        ...
        // 添加屏蔽关键词
          ...
          if (!!magicJS.request.body) {
            ...
            // 读取关键词
            let keyword = decodeURIComponent(magicJS.request.body).match(
              /keyword=(.*)/
            )[1];
            let keywords = magicJS.read(keywordBlockKey, userInfo.id);
            if (!keywords) {
        ...
        // 删除屏蔽关键词
          ...
          magicJS.request.method === 'DELETE' &&
          userInfo.is_vip !== true
        ) {
          let keyword = decodeURIComponent(magicJS.request.url).match(
            /keyword=(.*)/
          )[1];
          let keywords = magicJS.read(keywordBlockKey, userInfo.id);
          if (!keywords) {
...
function MagicJS(scriptName="MagicJS",logLevel="INFO")
  {
    return new class{constructor()
      {
        if(this.version="2.2.3.3",this.scriptName=scriptName,
        this.logLevels={DEBUG:5,INFO:4,NOTIFY:3,WARNING:2,ERROR:1,CRITICAL:0,NONE:-1},  
        this.isLoon="undefined"!=typeof $loon,  
        this.isQuanX="undefined"!=typeof $task,
        this.isJSBox="undefined"!=typeof $drive,
        this.isNode="undefined"!=typeof module&&!this.isJSBox,
        this.isSurge="undefined"!=typeof $httpClient&&!this.isLoon,
        this.node={request:void 0,fs:void 0,data:{}},
        ...
        this.logLevel=logLevel,this._barkUrl="",
        this.isNode)
          {
        ...
        else this.isJSBox&&($file.exists("drive://MagicJS")||$file.mkdir("drive://MagicJS"),
        $file.exists("drive://MagicJS/magic.json")||$file.write({data:$data({string:"{}"}),path:"drive://MagicJS/magic.json"}))}
        ...
        get response(){return"undefined"!=typeof $response?($response.hasOwnProperty("status")&&($response.statusCode=$response.status),
        $response.hasOwnProperty("statusCode")&&($response.status=$response.statusCode),
        $response):void 0}
        ...
        read(key,session=""){
          let val="";this.isSurge||this.isLoon?val=$persistentStore.read(key):this.isQuanX?val=$prefs.valueForKey(key):this.isNode?val=this.node.data:this.isJSBox&&(val=$file.read("drive://MagicJS/magic.json").string);
          try{this.isNode&&(val=val[key]),this.isJSBox&&(val=JSON.parse(val)[key]),session&&("string"==typeof val&&(val=JSON.parse(val)),val=val&&"object"==typeof val?val[session]:null)}
          catch(err){this.logError(err),val=session?{}:null,this.del(key)}
          void 0===val&&(val=null);try{val&&"string"==typeof val&&(val=JSON.parse(val))}catch(err){}
